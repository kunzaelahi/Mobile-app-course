import 'package:flutter/material.dart';

void main() {
  runApp(MyApp());
}

class MyApp extends StatelessWidget {
  // This widget is the root of your application.
  @override
  Widget build(BuildContext context) {
    return MaterialApp(
      debugShowCheckedModeBanner: false,
      home: Scaffold(
        appBar: AppBar(
          title:  Center(child: Text("Ecom App UI"),),
          actions: <Widget>[IconButton( icon: Icon(
                             Icons.notifications_active,
                              color: Colors.white, ),
                              onPressed: () {},)],
      
        ),


        body: Container(
          color: Colors.white,
          child: Column(children: [
            Container(
              color: Colors.white,
               height: 200,
                margin: EdgeInsets.all(10),
              child:Row(children: [

                Container(
                   height: 200,
                  width:190,
                  decoration: BoxDecoration(
                   borderRadius: BorderRadius.circular(10),
                    image: DecorationImage(
                    image: NetworkImage('data:image/png;base64,'),
                    fit: BoxFit.cover,
                    ),
                    ),
                ),
                       
Container(
  padding: EdgeInsets.only(top:40),
  alignment: Alignment.centerLeft,
  child:   Column(children:[
  
      Text("User",style: TextStyle(fontFamily: "arial",fontWeight: FontWeight.bold ,fontSize: 30,) ,),
  
      Text("User@gmail.com",style: TextStyle(fontFamily: "arial",fontWeight: FontWeight.bold ,fontSize: 17,) ,),
  
  SizedBox(height: 30,),
  Text("Logout",style: TextStyle(fontSize: 15,color:Colors.purple[300] ,fontFamily: "Arial",),)
  
    ]),
),
                 
                 ],
                ) ,
                ),
          
          

 Column(
   children: [
     
   Align(alignment:Alignment.bottomLeft, child: Text("  Account Information",style: TextStyle(fontSize: 25,color:Colors.black87,fontWeight: FontWeight.bold, fontFamily: "Arial",),)),


     Container(
       color: Colors.white,
       margin: EdgeInsets.only(left:20),
       child: ListTile(
           title: Text("Full name",style: TextStyle(fontSize: 20 ,fontFamily: "arial",fontWeight: FontWeight.bold,),),
           subtitle: Text("User",style: TextStyle(fontSize: 15,color:Colors.grey ,fontFamily: "arial",),) ,
           trailing: Container(child: Icon(Icons.edit, size:20, color:Colors.grey),),
           
       ),
     ),

      Container(
       color: Colors.white,
       margin: EdgeInsets.only(left:20),
       child: ListTile(
           title: Text("Email",style: TextStyle(fontSize: 20 ,fontFamily: "arial",fontWeight: FontWeight.bold,),),
           subtitle: Text("User@gmail.com",style: TextStyle(fontSize: 15,color:Colors.grey ,fontFamily: "arial",),) ,
           
       ),
     ),

Container(
       color: Colors.white,
       margin: EdgeInsets.only(left:20),
       child: ListTile(
           title: Text("Phone no",style: TextStyle(fontSize: 20 ,fontFamily: "arial",fontWeight: FontWeight.bold,),),
           subtitle: Text("User@gmail.com",style: TextStyle(fontSize: 15,color:Colors.grey ,fontFamily: "arial",),) ,
           
       ),
     ),


     Container(
       color: Colors.white,
       margin: EdgeInsets.only(left:20),
       child: ListTile(
           title: Text("Address",style: TextStyle(fontSize: 20 ,fontFamily: "arial",fontWeight: FontWeight.bold,),),
           subtitle: Text("New York, Rondom street, H no 72",style: TextStyle(fontSize: 15,color:Colors.grey ,fontFamily: "arial",),) ,
           
       ),
     ),

     Container(
       color: Colors.white,
       margin: EdgeInsets.only(left:20),
       child: ListTile(
           title: Text("Gender",style: TextStyle(fontSize: 20 ,fontFamily: "arial",fontWeight: FontWeight.bold,),),
           subtitle: Text("Male",style: TextStyle(fontSize: 15,color:Colors.grey ,fontFamily: "arial",),) ,
           
       ),
     ),

   ],
 )


          
          
          
          ], ),
        ),



      ),
      );
  }
}
